...
export default {
    ...
    mounted(){
        ...
        var _this = this;
        setTimeout(()=>{
            _this.$axios.get('/api/getLocation').then((res)=>{
                var msg = res.data.msg;
                if(msg === "ok"){
                    var nm = res.data.data.nm;
                    var id = res.data.data.id;
                    if(!id || id == _this.$store.state.city.id) {return; }
                    messageBox({
                        title: '定位',
                        content: nm,
                        cancel: '取消',
...
.movie_menu .hot_item{font-size: 15px; color: #666; width: 80%; text-align: center; margin: 0 12px; font-weight: 700;}
